 ...
 */
static int do_slabs_newslab(struct persistent_engine *engine, const unsigned int id);
static void *memory_allocate(struct persistent_engine *engine, size_t size);
...
unsigned int slabs_clsid(struct persistent_engine *engine, const size_t size) {
 ...
 * accordingly.
 */
ENGINE_ERROR_CODE slabs_init(struct persistent_engine *engine,
                             const size_t limit,
                             const double factor,
                             const bool prealloc) {
...
static int grow_slab_list (struct persistent_engine *engine, const unsigned int id) {
...
/*@null@*/
static void *do_slabs_alloc(struct persistent_engine *engine, const size_t size, unsigned int id) {
    slabclass_t *p;
    void *ret = NULL;
...
static void do_slabs_free(struct persistent_engine *engine, void *ptr, const size_t size, unsigned int id) {
    slabclass_t *p;
...
/*@null@*/
static void do_slabs_stats(struct persistent_engine *engine, ADD_STAT add_stats, const void *cookie) {
    int i, total;
...
void *slabs_alloc(struct persistent_engine *engine, size_t size, unsigned int id) {
    void *ret;
...
void slabs_free(struct persistent_engine *engine, void *ptr, size_t size, unsigned int id) {
    pthread_mutex_lock(&engine->slabs.lock);
    do_slabs_free(engine, ptr, size, id);
    pthread_mutex_unlock(&engine->slabs.lock);
}

void slabs_stats(struct persistent_engine *engine, ADD_STAT add_stats, const void *c) {
    pthread_mutex_lock(&engine->slabs.lock);
