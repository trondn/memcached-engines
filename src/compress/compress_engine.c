...
ENGINE_ERROR_CODE create_instance(uint64_t interface,
   ...
   struct compress_engine *engine = malloc(sizeof(*engine));
   if (engine == NULL) {
      return ENGINE_ENOMEM;
   }

   struct compress_engine compress_engine = {
      .engine = {
         .interface = {
            .interface = 1
         },
...
static inline struct compress_engine* get_handle(ENGINE_HANDLE* handle) {
   return (struct compress_engine*)handle;
}
...
static ENGINE_ERROR_CODE default_initialize(ENGINE_HANDLE* handle,
                                            const char* config_str) {
   struct compress_engine* se = get_handle(handle);
